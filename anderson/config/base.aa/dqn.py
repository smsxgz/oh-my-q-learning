...
def dqn(env,
    ...
    try:
        ...
        for i in range(num_iterations):
            ...
            # update
            summaries = estimator.update(discount_factor, *memory_buffer.sample(batch_size))
            results_buffer.update_summaries(summaries)
            total_t += 1

            if total_t % update_target_every == 0:
                estimator.target_update()
                results_buffer.update_summaries(estimator.alpha_update(discount_factor, *memory_buffer.sample(alpha_batch_size)))
