...
def update_target(sess, q, target, rho):
    e2_params = [
        t for t in tf.trainable_variables() if t.name.startswith(target.scope)
    ]
    e2_params = sorted(e2_params, key=lambda v: v.name)
    name_list = [v.name.replace(target.scope, q.scope) for v in e2_params]
    ...
    for e1_v, e2_v in zip(e1_params, e2_params):
        op = e2_v.assign((e1_v - e2_v) * rho + e2_v)
