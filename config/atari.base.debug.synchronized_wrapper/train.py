...
def main(game_name, lr, num_agents, update_target_every, model_name):
    ...
    # env = Agent(num_agents, get_atari_env_fn(game_name), basename, backend='multiprocessing')
    env = Agent(num_agents, get_atari_env_fn(game_name), basename, backend='ray')
    try:
        action_n = int(env.action_space.nvec[0])
        estimator = get_estimator(model_name, action_n, lr, 0.99)
